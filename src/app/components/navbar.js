        ...
        <ul className="hidden lg:flex gap-12 text-[#000] font-medium font-ui text-sm">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative flex items-center after:absolute after:bottom-0 after:left-0 after:top-[45px] after:w-full after:h-[3px] after:transition-all after:duration-200 after:pointer-events-none
                ${
                  pathname === link.href
